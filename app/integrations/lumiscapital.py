...
class LumiscapitalClient:
    """
    Wraps the Financial Modeling Prep stable API.
    ...
    async def get_historical_prices(self, symbol: str, days: int = 30) -> list[dict]:
        ...
        data = await self._get(f"/historical-price-eod/full", symbol=symbol.upper(), from_=from_date)
    ...
    async def get_economic_indicators(self) -> list[dict]:
        indicators = []
        for name in ["GDP", "federalFunds", "CPI", "inflationRate", "unemploymentRate",
                     "retailSales", "consumerSentiment", "realGDP"]:
            data = await self._get("/economic-indicators", name=name, limit=1)
            if data and isinstance(data, list) and data:
                indicators.append({"indicator": name, **data[0]})
        return indicators

    async def get_treasury_rates(self) -> Optional[dict]:
        data = await self._get("/treasury", limit=1)
        if data and isinstance(data, list):
            return data[0]
        return None

    async def get_sector_performance(self) -> list[dict]:
        return await self._get("/sector-performance") or []
    ...
    async def get_earnings_calendar(self, days_ahead: int = 7) -> list[dict]:
        ...
        data = await self._get("/earning-calendar", from_=from_date, to=to_date)
        return data or []

    async def get_earnings_surprises(self, symbol: str) -> list[dict]:
        data = await self._get("/earnings-surprises", symbol=symbol.upper())
        return (data or [])[:8]
    ...
    async def get_key_metrics(self, symbol: str) -> Optional[dict]:
        data = await self._get("/key-metrics", symbol=symbol.upper(), limit=1)
        if data and isinstance(data, list):
            return data[0]
        return None

    async def get_analyst_estimates(self, symbol: str) -> list[dict]:
        data = await self._get("/analyst-estimates", symbol=symbol.upper(), limit=4)
        return data or []
    ...
    async def get_stock_news(self, symbol: str, limit: int = 10) -> list[dict]:
        data = await self._get("/news/stock", symbol=symbol.upper(), limit=limit)
        return data or []

    async def get_political_news(self, limit: int = 10) -> list[dict]:
        data = await self._get("/senate-trading", limit=limit)
        return data or []
    ...
    async def get_gainers(self) -> list[dict]:
        return await self._get("/market/gainers") or []

    async def get_losers(self) -> list[dict]:
        return await self._get("/market/losers") or []

    async def get_most_active(self) -> list[dict]:
        return await self._get("/market/actives") or []
    ...
    async def get_insider_trades(self, symbol: str) -> list[dict]:
        data = await self._get("/insider-trading", symbol=symbol.upper(), limit=10)
        return data or []
    ...
    # keep market_risk_premium for compatibility
    async def get_market_risk_premium(self) -> list[dict]:
        return []

    async def get_price_target(self, symbol: str) -> list[dict]:
        data = await self._get("/price-target", symbol=symbol.upper(), limit=5)
        return data or []

    async def get_upgrades_downgrades(self, symbol: str) -> list[dict]:
        data = await self._get("/upgrades-downgrades", symbol=symbol.upper(), limit=10)
        return data or []

    async def get_income_statement(self, symbol: str) -> Optional[dict]:
        data = await self._get("/income-statement", symbol=symbol.upper(), limit=1)
        if data and isinstance(data, list):
            return data[0]
        return None
